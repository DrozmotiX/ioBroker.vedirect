...
class Vedirect extends utils.Adapter {
   ...
   */
  constructor(options) {
    // @ts-expect-error super call typing
    super({
      ...options,
      name: "vedirect",
    });
  ...
  async onReady() {
    ...
    this.log.info(
      "Starting VE.Direct with Protocol Version 3.33 and configurable expiring state capability",
    );
    ...
    try {
      ...
      port.on("error", (error) => {
        this.log.error(
          `Issue handling serial port connection : ${JSON.stringify(error)}`,
        );
        this.setState("info.connection", false, true);
      });
      ...
      parser.on("data", (data) => {
        ...
        if (!bufferMessage) {
          ...
          if (this.config.messageBuffer > 0) {
            this.log.debug(
              `Activate Message buffer with delay of ${this.config.messageBuffer * 1000}`,
            );
            bufferMessage = true;
            if (timeouts["mesageBuffer"]) {
              this.clearTimeout(timeouts["mesageBuffer"]);
              timeouts["mesageBuffer"] = null;
            }
            timeouts["mesageBuffer"] = this.setTimeout(() => {
              bufferMessage = false;
              this.log.debug(
                `Message buffer timeout reached, will process data`,
              );
  ...
  async parse_serial(line) {
    try {
      ...
      if (stateAttr[res[0]] !== undefined) {
        switch (
          res[0] // Used for special modifications to write a state with correct values and types
        ) {
          case "CE":
          ...
          case "AR":
            this.stateSetCreate(
              res[0],
              res[0],
              await this.get_alarm_reason(res[1]),
            );
            break;
          ...
          case "OR":
            this.stateSetCreate(
              res[0],
              res[0],
              await this.get_off_reason(res[1]),
            );
            break;
          ...
          case "ERR":
            this.stateSetCreate(
              res[0],
              res[0],
              await this.get_err_state(res[1]),
            );
            break;
          ...
          case "PID":
            this.stateSetCreate(
              res[0],
              res[0],
              await this.get_product_longname(res[1]),
            );
            break;
          ...
          case "MODE":
            this.stateSetCreate(
              res[0],
              res[0],
              await this.get_device_mode(res[1]),
            );
            break;
          ...
          case "MPPT":
            this.stateSetCreate(
              res[0],
              res[0],
              await this.get_mppt_mode(res[1]),
            );
            break;
          ...
          case "MON":
            this.stateSetCreate(
              res[0],
              res[0],
              await this.get_monitor_type(res[1]),
            );
            break;
   ...
   *
   * @param {() => void} callback Callback that must be called on completion
   */
  onUnload(callback) {
    this.setState("info.connection", false, true);
    try {
  ...
  stateSetCreate(stateName, name, value) {
    ...
    try {
      ...
      this.log.debug(
        `[stateSetCreate] state attribute from lib ${JSON.stringify(
          stateAttr[name],
        )}`,
      );
      common.name =
      ...
      common.type =
        stateAttr[name] !== undefined
          ? stateAttr[name].type || typeof value
          : typeof value;
      common.role =
        stateAttr[name] !== undefined
      ...
      if (
        ...
        this.log.debug(
          `[stateSetCreate] An attribute has changed for : ${stateName}`,
        );
        ...
        this.log.debug(
          `[stateSetCreate] No attribute changes for : ${stateName}, processing normally`,
        );
      }
      ...
      this.log.debug(
        `[stateSetCreate] All createdStatesDetails${JSON.stringify(
  ...
  sendSentry(msg) {
    if (!disableSentry) {
      this.log.info(
        `[Error catched and send to Sentry, thank you collaborating!] error: ${msg}`,
      );
